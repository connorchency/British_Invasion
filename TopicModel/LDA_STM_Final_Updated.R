...
BA = rbind.data.frame(AM_New, BI_New, stringsAsFactors = Falase)
...
# ly_BA_new = dfm_trim(ly_BA, verbose = T)
rowTotals <- apply(ly_BA_new , 1, sum)
#colTotals <- apply(ly , 2, sum)
ly_BA_new  = ly_BA_new[rowTotals> 0, ]
...
mod2<-LDA(ly_BA_new, k = 10, method = "Gibbs",  control = list(seed = 826))

# Quickly extracts the word weights and transforms them into a data frame
topics <- tidy(mod2, matrix = "beta")
# Generates a df of top terms
top_terms <- topics %>%
  group_by(topic) %>%
  top_n(20, beta) %>%
...
# Store the results of the distribution of topics over documents
doc_topics<-mod2@gamma
# Store the results of words over topics
words_topics<-mod2@beta
...
BI_AM_2 = BA[rowTotals> 0,]
BI_AM_2 = rbind.data.frame(BI_AM_2[max1,],BI_AM_2[max2,],
                           BI_AM_2[max3,])#,
                           #BI_AM_2[as.vector(max4),],
                           #BI_AM_2[as.vector(max5),])
BI_AM_2$Top_Topic=seq(1:10)

# 
BI_AM_3 = BA[which(rowTotals> 0),]
#BI_AM_3 = cbind.data.frame(apply(doc_topics, 1, which.max), BI_AM_3)
BI_topic_distr = table(apply(doc_topics, 1, which.max)[BI_AM_3$Label==1])/sum(BI_AM_3$Label==1)
AM_topic_distr = table(apply(doc_topics, 1, which.max)[BI_AM_3$Label==3])/sum(BI_AM_3$Label==3)
plot(AM_topic_distr, col=2, pch=3, type = 'o',lwd = 0.5, ylim = c(0,0.2))
points(BI_topic_distr, col=3, pch=3, type = 'o',lwd =0.5)
...
corpus_gt<-textProcessor(documents=BA_stm$Lyrics,
                         metadata = BA_stm[,c("Artist","Label","Lyrics")],
                         language='english',
